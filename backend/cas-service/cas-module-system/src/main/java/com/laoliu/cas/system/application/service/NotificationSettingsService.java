 ...
 *
 * <p>存储约定（见 cas-service/sql/02_user.sql）：
 * <ul>
 *   <li>管理端全局策略：notification_policy 单行表（id=1）的 email_enabled / sms_enabled</li>
 *   <li>用户个人偏好：user 表的 email_notify 列</li>
 * </ul>
 * 实际发送 = 管理端策略启用 且 用户偏好开启。站内消息未实现，无对应通道。</p>
 *
 * @author forever-king
 */
@Service
